...
def generate_requests_data(num_reqs, num_ts, num_nodes):
    ...
    while it < num_reqs:
        ...
        requests_df = requests_df._append(curr_req, ignore_index=True)
        it += 1

        '''producing batches'''
        clubbed_order = random.randint(0, 3)
        if(clubbed_order == 1):
            # next_order_cust_node = cust_node + clubbed_order
            # rest_node = random.randint(1, num_nodes)
            cust_node = -1
            while cust_node==rest_node or cust_node==-1:
                cust_node = random.randint(1, num_nodes)
            ...
            # choosing time
            # prep_ts = prep_timestamps[it]
            prep_ts += 10*clubbed_order
            placed_ts += 7*clubbed_order
            # placed_ts +
            deliver_ts += 11*clubbed_order
            prep_time = prep_ts - placed_ts
            deliver_time = deliver_ts - prep_ts
            deliver_dist = speed * deliver_time
